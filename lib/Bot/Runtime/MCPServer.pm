...
sub _tool_specs {
  ...
  return [
    ...
    },
    {
      ...
      description  => 'Executes a SPARQL query against a local Jena Fuseki database. CRITICAL RULES: 1. Use known classes only when you are confident the class is correct: Theater (wd:Q24354), Cinema (wd:Q41253), Theme Park (wd:Q194195), Museum (wd:Q33506), Castle/Château/Fort/Palace in the current France castle graph (wd:Q23413). Do not invent or guess type IDs. 2. LABELS: data may have @en and/or @fr labels. Do not assume English-only labels. Use `STR()` and `LCASE()`, for example `FILTER(CONTAINS(LCASE(STR(?cityLabel)), "marseille"))`, and prefer `FILTER(LANG(?label) IN ("en", "fr"))` when fetching labels. 3. NAME MATCHING: accented and unaccented spellings may differ (for example `château` vs `chateau`, `Pastré` vs `Pastre`). If an accented exact-name query returns empty, do at most one broader follow-up using lowercase string matching on labels, and consider both accented and unaccented variants. 4. PROPERTIES: wdt:P131 (location), wdt:P571 (date), wdt:P84 (architect), wdt:P625 (GPS), wdt:P1435 (heritage status). Use OPTIONAL for dates, architects, and heritage. 5. GEOSPATIAL: Coordinates (P625) look like "Point(Long Lat)". Use `replace(str(?coords), "Point\\\\((.*) (.*)\\\\)", "$2")` to get Latitude as a number. 6. SEARCH: do not rely on guessed Wikidata IDs for locations. Prefer filtering locations by label string using rdfs:label. 7. LABEL FETCHING: NEVER use `SERVICE wikibase:label`. Always fetch `rdfs:label` manually. 8. QUERY DISCIPLINE: If an exact class-based query returns empty or obviously wrong rows, do at most one broader follow-up query. Do not keep iterating similar queries. 9. NAMED-ENTITY CASTLE STRATEGY: for a named castle/château/fort/palace, do this in two steps when needed: (a) identify the item by name + location using labels; then (b) query details for that matched item, including OPTIONAL architect/date/coords/heritage. Do not stop at a fuzzy discovery query if you already found a likely item. 10. ARCHITECT HANDLING: architect existence is determined by a raw `?item wdt:P84 ?architect` binding. Query `?architect` directly first. `?architectLabel` or name fields are optional enrichment only. If `?architect` exists but no readable label/name is available, report that architect data exists in the graph but a human-readable architect label is unavailable. Multiple architect rows are allowed. Deduplicate repeated rows caused by multiple labels or heritage values. Do not say architect data is missing unless no `wdt:P84` bindings are returned for the matched item. 11. CASTLE RESPONSE RULE: if the entity is found but architect data is missing, say that clearly. If results are empty or irrelevant, say you could not verify a reliable match from the database instead of guessing. 12. NAMED-ENTITY VERIFICATION RULE (all venue types): when a user asks about a specific named place (for example a particular theater), you MUST run at least one Fuseki query in the current turn before claiming it is missing/not in the database. If your first query is empty, run one broader follow-up query (for example name-only or location-only) before concluding absence. 13. IMPORTANT: Always start your final response by addressing the user (e.g., "mateu: I found...").',
      input_schema => {
        type       => 'object',
        properties => {
