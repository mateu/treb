...
sub _tool_specs {
  ...
  return [
    ...
    },
    {
      ...
      description  => 'Executes a SPARQL query against a local Jena Fuseki database. CRITICAL RULES: 1. Use `a` (rdf:type) for known classes only when you are confident the class is correct: Theater (wd:Q24354), Cinema (wd:Q41253), Theme Park (wd:Q194195), Museum (wd:Q33506), Castle/Château (wd:Q77148). Do not invent or guess type IDs. 2. ALL labels have "@en" tag; use `STR()`: `FILTER(CONTAINS(LCASE(STR(?cityLabel)), "marseille"))`. 3. PROPERTIES: wdt:P131 (location), wdt:P571 (date), wdt:P84 (architect), wdt:P625 (GPS), wdt:P1435 (heritage status). 4. Use OPTIONAL for dates, architects, and heritage. 5. GEOSPATIAL: Coordinates (P625) look like "Point(Long Lat)". Use `replace(str(?coords), "Point\\\\((.*) (.*)\\\\)", "$2")` to get Latitude as a number. 6. SEARCH: NEVER use Wikidata IDs (e.g., wd:Q23431) for locations. ALWAYS filter by city name string using rdfs:label. 7. LABELS: NEVER use "SERVICE wikibase:label". Always manually fetch rdfs:label and use FILTER(LANG(?label) = "en"). 8. QUERY DISCIPLINE: If an exact class-based query returns empty or obviously wrong rows, do at most one broader follow-up query. Do not keep iterating similar queries. 9. CASTLE-SPECIFIC RULE: For castle/château/fort/palace questions, if results are empty, irrelevant, or lack architect data, stop and tell the user you could not verify a reliable match from the database instead of guessing. 10. IMPORTANT: Always start your final response by addressing the user (e.g., "mateu: I found...").',
      input_schema => {
        type       => 'object',
        properties => {
