...
my %PERSONA_TRAIT_META = (
  ...
  bot_reply_pct => { kind => 'pct', env => 'BOT_REPLY_PCT', default => 25 },
...
use Bot::Runtime::WebTools qw(format_search_results summarize_url search_web);
...
sub _summarize_url {
  ...
  return summarize_url(
    url => $url,
    summarize_special_url_cb => sub { $self->_summarize_special_url(@_) },
    raid_cb => sub { $self->_raider->raid(@_) },
  );
}
...
sub _search_web {
  ...
  return search_web(
    query => $query,
    limit => $limit,
    api_key => ($ENV{BRAVE_API_KEY} // q{}),
    format_search_results_cb => sub { $self->_format_search_results(@_) },
  );
}
